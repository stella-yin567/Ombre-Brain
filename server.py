...
async def auth_setup_endpoint(request):
    ...
    resp.set_cookie("ombre_session", token, httponly=True, samesite="lax", max_age=86400 * 7,path="/")
    return resp
    ...
    if transport in ("sse", "streamable-http"):
        ...
        class RootPathMiddleware:
            ...
            async def __call__(self, scope, receive, send):
                if scope["type"] == "http":
                    env_root = os.environ.get("ROOT_PATH", "")
                    if env_root:
                        scope["root_path"] = env_root.rstrip("/")
                    else:
                        headers = dict(scope.get("headers", []))
                        forwarded_prefix = headers.get(b"x-forwarded-prefix", b"").decode()
                        if forwarded_prefix:
                            scope["root_path"] = forwarded_prefix.rstrip("/")
                return await self.app(scope, receive, send)
        _app.add_middleware(RootPathMiddleware)
